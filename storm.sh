...
# Storm - Fast Bash Download Manager
#
...
function show_usage() {
  echo -e "\nUsage: $(basename "$0") <url> - Download file to the current folder."
  exit 1
}
...
function download_file() {
  if [[ -n $BIN_ARIA ]]; then
    if [[ $VERBOSE_MODE == true ]]; then
      aria2c --timeout=$MAX_TIMEOUT --max-tries=$MAX_RETRY --user-agent="$USER_AGENT" --referer="$1" --max-connection-per-server="$MAX_CON" --auto-file-renaming=false --file-allocation=none "$1"
    else
      aria2c --timeout=$MAX_TIMEOUT --max-tries=$MAX_RETRY --user-agent="$USER_AGENT" --referer="$1" --max-connection-per-server="$MAX_CON" --auto-file-renaming=false --file-allocation=none --console-log-level=error --summary-interval=0 --download-result=hide "$1"
    fi
  elif [[ -n $BIN_PGET ]]; then
    local PGET_MAX_CON ; [[ $MAX_CON -gt 4 ]] && PGET_MAX_CON=4 || PGET_MAX_CON=$MAX_CON
    pget -p "$PGET_MAX_CON" --timeout $MAX_TIMEOUT --user-agent "$USER_AGENT" --referer "$1" "$1"
    ...
    lftp --norc -c "set net:timeout $MAX_TIMEOUT;set net:max-retries $MAX_RETRY;set http:user-agent '$USER_AGENT';set xfer:log false;set http:referer '$1';pget -c -n $MAX_CON '$1' -o '$2';"
  elif [[ -n $BIN_CURL ]]; then
    if [[ $VERBOSE_MODE == true ]]; then
      curl --connect-timeout $MAX_TIMEOUT --retry $MAX_RETRY --user-agent "$USER_AGENT" --referer "$1" -OJSL "$1"
    else
      curl --connect-timeout $MAX_TIMEOUT --retry $MAX_RETRY --user-agent "$USER_AGENT" --referer "$1" -OJSL --progress-bar "$1"
    fi
  elif [[ -n $BIN_WGET ]]; then
    if [[ $VERBOSE_MODE == true ]]; then
      wget --timeout $MAX_TIMEOUT --tries $MAX_RETRY --user-agent "$USER_AGENT" --referer "$1" --content-disposition "$1"
    else
      wget --timeout $MAX_TIMEOUT --tries $MAX_RETRY --user-agent "$USER_AGENT" --referer "$1" --content-disposition --quiet --show-progress "$1"
    fi
  else
    die "Unable to find proper binary to handle download request."
  fi
}
